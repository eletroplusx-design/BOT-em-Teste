...
def _build_canonical_artifacts_root(tmp_path_factory: pytest.TempPathFactory) -> Path:
    root = tmp_path_factory.mktemp("phase48-canonical-artifacts")
...
def canonical_artifacts_root(tmp_path_factory: pytest.TempPathFactory) -> Path:
    return _build_canonical_artifacts_root(tmp_path_factory)


@pytest.fixture()
def canonical_verification(canonical_artifacts_root: Path) -> phase44.CanonicalOfflineResearchEvidenceVerification:
    return phase44.verify_canonical_offline_research_evidence_fixture(canonical_artifacts_root)


@pytest.fixture(scope="module")
def canonical_runtime_bundle(canonical_artifacts_root: Path) -> dict[str, object]:
    verification = phase44.verify_canonical_offline_research_evidence_fixture(canonical_artifacts_root)
    ...
    authorization = phase45.build_offline_research_execution_authorization(
        ...
        issued_at_utc=CANONICAL_AUTHORIZATION_ISSUED_AT_UTC,
    ...
    phase45.register_offline_research_execution_authorization(
        ...
        updated_at_utc=CANONICAL_AUTHORIZATION_ISSUED_AT_UTC,
    )
    ...
    result = phase47.execute_neutral_offline(
        request,
        started_at_utc=CANONICAL_RESULT_STARTED_AT_UTC,
        finished_at_utc=CANONICAL_RESULT_FINISHED_AT_UTC,
