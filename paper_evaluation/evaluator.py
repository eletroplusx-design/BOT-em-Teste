    ...
    evaluation_identity = evaluation_id or paper_evaluation_hash(
        {
            ...
            "period_start_utc": operational_batch.contract.period_start_utc if operational_batch is not None else ordered_evidence[0].session_started_utc if ordered_evidence else now,
            "period_end_utc": operational_batch.contract.period_end_utc if operational_batch is not None else ordered_evidence[-1].session_updated_utc if ordered_evidence else now,
            "strategy_version": operational_batch.contract.strategy_version if operational_batch is not None else ordered_evidence[0].strategy_version if ordered_evidence else "v8_paper_evaluation",
            "cohort_hash": operational_batch.contract.cohort_hash if operational_batch is not None else None,
        }
    )
    manifest = PaperEvaluationManifest(
        evaluation_id=evaluation_identity,
        period_start_utc=operational_batch.contract.period_start_utc if operational_batch is not None else accepted[0].session_started_utc if accepted else ordered_evidence[0].session_started_utc if ordered_evidence else now,
        period_end_utc=operational_batch.contract.period_end_utc if operational_batch is not None else accepted[-1].session_updated_utc if accepted else ordered_evidence[-1].session_updated_utc if ordered_evidence else now,
        inclusion_rule=operational_batch.contract.inclusion_rule if operational_batch is not None else inclusion_rule,
        ...
        policy_hash=policy.policy_hash,
        strategy_version=operational_batch.contract.strategy_version if operational_batch is not None else accepted[0].strategy_version if accepted else ordered_evidence[0].strategy_version if ordered_evidence else "v8_paper_evaluation",
